      ...
      select g.nome_genero,
       ...
       from genero g
        inner join genero_artista ga on g.id = ga.id_genero -- PK = FK
         ...
         --------- Mais exemplos de comando UPDATE------------
         
	  update artista 
	   set popularidade = 10000
		where nome like 'a%' or seguidores < 2000  -- '%' não importa o que vem depois da letra 'a'
		 or seguidores < 2000
             
        update artista 
	     set popularidade = 50000
          where nome like 'a%' or seguidores < 2000
           or seguidores < 2000
           
           
  like 'a%' -- começa com, não iporta o que vem depois --
  like '%a' -- termina com a(string), não importa o que vem antes --
  like '%a%' -- precisa ter 'a'(string) no meio da string --
  
         update artista 
	      set popularidade = 60000
           where nome = 'anita' -- sinal de '=' compara exatamente o valor da string --